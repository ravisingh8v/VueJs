<template>
    <section class="d-flex justify-content-center">
        ...
        <ul class="w-50" v-else>
            ...
            <li class="p-2 border border-secondary rounded my-2 d-flex justify-content-between align-items-center" v-for="domain in domains"
                :key="domain.id">
                ...
                <div>
                    <a class="btn btn-secondary" @click="viewMember(domain.name)">View Members</a>
                </div>
            </li>
        </ul>
    </section>
    ...
    watch: {
        employeeList() {
            for (let domain in this.domains) {
                ...
                let memberList = this.employeeList.filter((res) => res.domain === this.domains[domain].name)
